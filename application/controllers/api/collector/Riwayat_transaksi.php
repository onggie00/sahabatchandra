...
class Riwayat_transaksi extends REST_Controller {
  ...
  function index_get() {
      ...
      if(isset($headers['token']))
        $token =  $headers['token'];
        $cek_token = $this->mymodel->getbywhere("collector",'token',$token,'row');

        if (empty($cek_token)) {
          $msg = array('success'=>0,'message'=>'Token tidak ditemukan',"data"=>array());
        }else{
          $data = $this->mymodel->getbywhere('trans_agen',"status_transaksi","1","result");
          foreach ($data as $key => $value) {
            ...
            $value->jenis_transaksi = "Penerimaan Uang ";
