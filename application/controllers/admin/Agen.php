...
class Agen extends CI_Controller {
  ...
  public function index($val = "")
  {
    ...
    $data['title_page'] = "Manajemen Agen";
  ...
  {
    $this->load->model(array('Agen_datatable'));
    $fetch_data = $this->Agen_datatable->make_datatables();
           $data = array();
           $nomor=1;
           foreach($fetch_data as $value)
           {
                ...
                $sub_array[] = $value->total_uang;
                $sub_array[] = $value->alamat;
                /*if (!empty($value->img_file)) {
                  $sub_array[] ='<img src="'.base_url("assets/image/agen/".$value->img_file).'" alt="" width="150px" height="100px">';
                }*/
                ...
                $sub_array[] ='
                <button type="button" name="update" id="'.$value->id_agen.'" class="btn btn-sm btn-primary update">
                <i class="mdi mdi-pencil-circle ml-1"></i> Ubah</button>
                <br><br>
                <button type="button" name="delete" id="'.$value->id_agen.'" class="btn btn-sm btn-danger delete">
           ...
           $output = array(
                ...
                "recordsTotal"          =>      $this->Agen_datatable->get_all_data(),
                "recordsFiltered"     =>     $this->Agen_datatable->get_filtered_data(),
                "data"                    =>     $data
           );
  ...
  {
    $cek_username = $this->mymodel->getbywhere('agen','username',$_REQUEST['username'],'row');
    if (empty($cek_username)) {
      if ($_REQUEST['password'] == $_REQUEST['cpassword']) {
        /*if(!empty($_FILES['img']['name'])){
          $this->load->library('upload');
          $config['upload_path'] = './assets/image/agen/'; //path folder
          ...
          if ($this->upload->do_upload('img')){
              ...
              $config['source_image']='./assets/image/agen/'.$gbr['file_name'];
              $config['create_thumb']= FALSE;
              $config['maintain_ratio']= FALSE;
              $config['quality']= '100%';
              $config['width']= 500;
              $config['height']= 300;
              $config['new_image']= './assets/image/agen/'.$gbr['file_name'];
         ...
         if(!empty($data)){
            $in = $this->mymodel->insert('agen',$data);
            if ($in) {
  ...
  public function updatedata()
  {
    if(!empty($_FILES['img']['name'])){
          ...
          if ($this->upload->do_upload('img')){
              ...
              $cek = $this->mymodel->getbywhere('agen','id_agen',$_REQUEST['data_id'],'row');
              if ($cek->img_file!=null) {
                unlink('./assets/image/agen/'.$cek->img_file);
              }
              $gambar=$gbr['file_name'];
          $data = array(
              "nama_lengkap" => $_REQUEST['nama_lengkap'],
              ...
              "email" => $_REQUEST['email'],
              "img_file" => $gambar,
              "total_uang" => $_REQUEST['total_uang'],
              "username" => $_REQUEST['username']
            );
          if(!empty($data)){
            $up = $this->mymodel->update('agen',$data,'id_agen',$_REQUEST['data_id']);
      ...
      $this->mymodel->update("agen",$data,'id_agen',$_REQUEST['data_id']);
      $this->session->set_flashdata('success_msg','Data Berhasil Diubah');
    }
  ...
  public function deletedata()
  {
    ...
    $del = $this->mymodel->delete('agen','id_agen',$_REQUEST['data_id']);
    if ($del) {
  ...
  {
    $id = $_REQUEST['id'];
    $data["data"] = $this->mymodel->getbywhere('agen','id_agen',$id,"row");
    $this->load->view('admin/modal_edit/agen',$data);
  }
